...
void sleep_manager_init(void) {
    // Release GPIO holds from previous sleep first
...
void sleep_manager_enter_deep_sleep(fire_stage_t stage) {
    ...
    // Step 2 — Hold GPIO pins
    power_manager_hold_pins();

    // Step 3 — Configure wakeup
    esp_sleep_enable_timer_wakeup(sleep_us);

    // Step 4 — Enter deep sleep — does not return
...
void sleep_manager_enter_deep_sleep_with_soc(fire_stage_t stage,
    ...
    if (soc <= BATTERY_CRIT_SOC) {
        // Critical battery — sleep 30 min regardless of fire stage
        minutes = 30;
        ESP_LOGW("SLEEP",
                 "Battery CRITICAL (%d%%) — forced 30min sleep",
                 soc);
    } else if (soc <= BATTERY_LOW_SOC) {
        // Low battery — sleep longer than normal for current stage
        uint8_t normal = SLEEP_MINUTES[stage];
        minutes = (normal < 10) ? 10 : normal;
        ESP_LOGW("SLEEP",
                 "Battery LOW (%d%%) — extending sleep to %dmin",
                 soc, minutes);
    } else {
        // Normal battery — use standard stage sleep time
        g_current_stage = stage;
    ...
    ESP_LOGI("SLEEP", "Deep sleep: %dmin (stage=%s SOC=%d%%)",
